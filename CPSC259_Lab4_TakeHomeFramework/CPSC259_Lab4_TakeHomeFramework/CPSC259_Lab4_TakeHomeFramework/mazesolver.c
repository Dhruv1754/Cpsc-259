...
void generate_all_paths( maze_cell ** maze, int dimension, int row, int column, char * path )
{
    ...
    if (row<0 ||     // row or col is outside the grid
		column < 0 ||     //
		column > dimension - 1 ||     //
		row>dimension-1 ||     //
		maze[row][column].character == MAZE_WALL ||     // it's a wall or a point already visted
		maze[row][column].visited == VISITED)// INSERT CODE HERE: Simply return if we hit one of the base cases (there are more than 1)
...
int path_cost ( char * path_string )
{
	...
	while (count < length) cost += path_string[count++] - '0';
	return cost;
		// INSERT CODE HERE
}
...
void display_shortest_path ( )
{
	...
	char * shortest = (char*)malloc(100 * sizeof(char));//defines a temp current path and allocates memory for it
	shortest = *paths;

	for (counter = 0; counter < paths_found; counter++) {
		//compares all lengths
		if (strlen(*(paths + counter)) < length) { //if the length of the next path is less than the previous path

			shortest = *(paths + counter);//next path = path + count
			length = strlen(*(paths + counter));//get next length
		}
	}
...
void display_cheapest_path()
{
	...
	for (i = 0; i < paths_found; i++) { //for the number of paths for a given maze

		if (path_cost(*(paths + i)) < cost) { //compares previous cost with current cost

			cheapest_path = *(paths + i); //saves path
			cost = path_cost(*(paths + i)); //loads the lowest value (between previous and current costs) into current cost
		}
	}
