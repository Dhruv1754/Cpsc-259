...
void analyze_segments( char * sample_segment, char ** candidate_segments, int number_of_candidates )
{
	...
	while (j < number_of_candidates)
	{
		candidate_length = strlen(candidate_segments[j]);
		if (candidate_length == sample_length)
		{
			while (i < sample_length)
			{
				if (sample_segment[i] ==*( candidate_segments[j] + i))
					i++;
				else
					i = sample_length + 1;
			}
			if (i == sample_length)
			{
				printf("Canidate %d is a perfect match", (j + 1));
					has_perfect_match++;
				i = 0;
			}
		}
	...
	else
		{
			for (i = 0; i < number_of_candidates; ++i) {
				...
				score = calculate_score(sample_segment, candidate_segments[i]);
					printf("canidate # %d match with a score of %d", (i + 1), score);
			}
		}
	/* End of function */
	return;
...
int calculate_score( char * sample_segment, char * candidate_segment)
{
	...
	int temp_score = 0, i=0, j=0, k=0,l=0,counter, nucleotide = 0, amountofbasepair=0 ;
	int score      = 0;
	...
	int segment_difference = candidate_length_in_codons - sample_length_in_codons;
	char* adcandidateptr;
	char* adsampleptr;
	char * sample; 
	sample = (char*)malloc(3 * sizeof(char));
	char * candidate;
	candidate= (char*)malloc(3 * sizeof(char));
	while (iterations <= segment_difference)

	{
		i = 0;
		temp_score = 0;
		//runs a loop to get the points for 1 sample
		while (i < (sample_length/3)*3)
		{
			k = 0;
			counter = 0;
			nucleotide = 0;
			while ( nucleotide < CODON_LENGTH)
			{
				//if the segmesnts match then just add a counter of the match example A and A match thats 1 match
				if (sample_segment[i] == candidate_segment[j])
					counter++;
				//check every segments match in the nucletoide running throught
				sample[k] = sample_segment[i];
				candidate[k] = candidate_segment[j];//copy the match into the segmensts 
			...
			//getting the addresses of the sample and canidates for the functions
			adsampleptr = &codon_names[get_codon_index(sample)][0];
			adcandidateptr = &codon_names[get_codon_index(candidate)][0];
			//they are used to compare the to chars of strings in an statement later
			// all 3 nucleotides segments  were a match gives 10 score 
			if  (counter == 3)
			
				temp_score += 10;
			
			//adds 5 to score seeing a amino acid 
			else if (strncmp(adsampleptr, adcandidateptr, CODON_NAME_LENGTH) == 0)
			{
				temp_score += 5;
			}

			else 
			{//if there are 2 matches, score added 4
				if (counter == 2)
					temp_score += 4;
				else if (counter == 1)//1 match then add 2
					temp_score += 2;
				for (k = 0; k < CODON_LENGTH; k++)
				{
					value = is_base_pair(sample[k], candidate[k]);
		...
		//overwrites the score if its less then the previous score
		if (score < temp_score)
			score = temp_score;
